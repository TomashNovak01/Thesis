    ...
    <table class="fieldAct__table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="smt noBorder ta-l">
            <select v-model="selectedContract" class="smt noBorder ta-l w100 s30">
              <option v-for="(contract, index) of contracts" :key="'contract_' + index" :value="contract">
                {{ contract.value_short }}
              </option>
            </select>
          </td>
        ...
        <tr>
          ...
          <td class="smt noBorder ta-l" />
          <td class="smt noBorder ta-l">
            <input type="date" v-model="selectedGRPDate" class="table_input" />
          </td>
        </tr>
        <tr>
        ...
        <template v-for="(field, i) in [...data.data, propants]">
          <tr v-if="field" :key="'field_' + i" :class="{ lastSimpleRow: field.title === 'Всего пропанта' }">
            <td class="ta-l" colspan="6">
              <template v-if="!field.is_title_editable">{{ field.title }}</template>
              <input v-else :id="`input1-${i}`" v-model.number="field.title" class="table_input"
                @focus="setCurrent(i, 1)" />
            </td>
            <td class="smt ta-l">{{ field.unit }}</td>
            <td class="ta-r">
              <input :id="`input2-${i}`" v-model.number="field.mb_before_fracturing" class="table_input"
                @focus="setCurrent(i, 2)" @change="field.to_pay = getToPay(field)" />
            </td>
            <td class="ta-r">
              <input :id="`input3-${i}`" v-model.number="field.mb_after_fracturing" class="table_input"
            ...
            <td class="ta-r">
              <input :id="`input5-${i}`" v-model.number="field.design" class="table_input" @focus="setCurrent(i, 5)" />
            </td>
            <td class="ta-r">
              <input :id="`input6-${i}`" v-model.number="field.redesign" class="table_input" @focus="setCurrent(i, 6)" />
            </td>
            <td class="ta-r">
              <input :id="`input7-${i}`" v-model.number="field.fact" class="table_input" @focus="setCurrent(i, 7)" />
            </td>
            <td class="ta-r">{{ Math.round(((field.redesign || 0) - (getFactMB(field) || 0)) * 1000) / 1000 }}</td>
            <td>{{ getPercentage(field) }}</td>
            <td class="ta-r">
              <input :id="`input10-${i}`" v-model.number="field.to_pay" class="table_input" @focus="setCurrent(i, 10)" />
            </td>
            <td class="noBorder hideOnPrint" />
            ...
            <td>
              <button @click="data.data.splice(i, 1)" class="delete">❌</button>
            </td>
          </tr>
...
import contracts from "../../../files/contracts.json";
...
export default {
  ...
  setup(props) {
    const selectedContract = ref({ value_short: "", date: "" });

    for (const contract of contracts)
      if (contract.id_code === props.data.id_contract) selectedContract.value = contract

    const selectedGRPDate = ref(dayjs(props.data.grp_date, "DD.MM.YYYY").format("YYYY-MM-DD"));
